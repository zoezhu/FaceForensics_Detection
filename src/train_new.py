...
from utils import set_loger, freeze_until, model_validation, set_lr
...
parser.add_argument("--freeze_until_layer", type=str, default=None)  #"layer2.0.conv1.weight")
parser.add_argument("--batch_size", type=int, default=128)
parser.add_argument("--num_workers", type=int, default=16)
parser.add_argument("--epochs", type=int, default=20)
# set grid attribute
parser.add_argument("--d1", type=float, default=16, help="遮挡块的大小")
parser.add_argument("--d2", type=float, default=160, help="含有遮挡块的区域")
...
## Get data
data_pickle_path = "../dataset/neural_original_all.pkl"
...
def lrfn(optimizer, epoch):
    LR_START = 1e-5
    LR_MAX = 5e-5  #* args.batch_size/128  # 这个可以调大一点
    LR_MIN = 1e-5
    LR_RAMPUP_EPOCHS = 5
    LR_SUSTAIN_EPOCHS = 0
    LR_EXP_DECAY = .8
...
for epoch in range(args.epochs):
    ...
    for itr, (data, label) in enumerate(dataloader_train):
        ...
        if not itr%50:
            ...
            val_loss, val_acc, real_acc = model_validation(net, dataloader_val, criterion)
            writer.add_scalar("Loss/val", val_loss, step-1)
            writer.add_scalar("Acc/val", val_acc, step-1)
            writer.add_scalar("RealAcc/val", real_acc, step-1)
    ...
    logger.info('[EPOCH %d] train loss:%.8f, val loss:%.8f, val acc:%.8f, real acc:%.8f' % (epoch, running_loss/total_bz, val_loss, val_acc, real_acc))
